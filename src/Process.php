...
class Process
{
    ...
    {   
        ...
        /*
         * master.pid 旧文件记录 master 进程 pid, 方便之后进程管理
         * 请管理好此文件位置, 使用 systemd 管理进程时会用到此文件
         * 判断文件是否存在，并判断进程是否在运行
         */
        if (file_exists($this->pidFile)) {
            //'master.info'; //pid 序列化信息
        ...
        //获取当前启动的PHP（主进程）进程的ID
        $this->ppid    = getmypid();
        $data['pid']   =$this->ppid;
        $data['status']=$this->status;
        //保存主进程到文件master.pid文件和master.info文件
    ...
    public function registSignal()
    {
        ...
        //外子进程退出的时候会向其父进程发送一个SIGCHLD信号
        //子进程退出后，父进程务必要执行Process::wait进行回收，否则这个子进程就会变为僵尸进程
        //父进程可以设置监听SIGCHLD信号，收到信号后执行Process::wait回收退出的子进程。
        \Swoole\Process::signal(SIGCHLD, function ($signo) {
            while (true) {
                try {
                    //在异步信号回调中执行wait 必须为false，非阻塞模式
    ...
    private function killWorkersAndExitMaster()
    {
        ...
        if ($this->workers) {
            foreach ($this->workers as $pid => $worker) {
                //强制杀workers子进程 默认的信号为SIGTERM，表示终止进程
